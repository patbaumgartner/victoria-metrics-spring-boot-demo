...
- `otel-collector/otel-collector-config.yml` — OTLP gRPC/HTTP receivers, `memory_limiter` + `batch` processors, and three `otlp_http` exporters (one per signal).
- `grafana/provisioning/datasources/datasources.yml` — provisions the three datasources.
