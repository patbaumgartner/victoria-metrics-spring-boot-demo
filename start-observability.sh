...
services=("victoria-metrics" "victoria-logs" "victoria-traces" "loki" "grafana" "otel-collector")
...
until curl -sf http://localhost:3100/ready > /dev/null 2>&1; do sleep 1; done
echo -e " ${GREEN}✓${NC}"
...
echo ""
echo -e "${BLUE}▶️  Start the demo app on your host:${NC} cd app && ./mvnw spring-boot:run"
