...
Collector, which fans each signal out to purpose-built backends and visualizes
everything in Grafana — no Prometheus scraping, no vendor agents.
...
- **Purpose-built storage** — VictoriaMetrics (metrics), VictoriaLogs (logs),
  VictoriaTraces (traces, Jaeger-compatible query API), and Loki for Grafana
  Logs Drilldown compatibility.
- **Auto-provisioned Grafana** — datasources and dashboards load on startup.
- **Run anywhere** — the observability stack runs in Docker (`docker compose up -d`);
  the Spring Boot app runs on your host with `./mvnw spring-boot:run`.
...
flowchart LR
    ...
    Col -- "metrics" --> VM
    Col -- "logs" --> VL
    Col -- "logs (drilldown compatibility)" --> LK
    Col -- "traces" --> VT
    VM --> GF
    VL --> GF
    VT --> GF
...
2. Run the Spring Boot app on your host (it pushes telemetry to the collector on
   `localhost:4318`, the default OTLP endpoint):
...
```bash
docker compose down        # stop the observability stack
docker compose down -v     # stop and remove volumes (reset all data)
```

Stop the app on the host with `Ctrl+C` in its terminal.
...
├── observability/
│   ├── otel-collector/               # Collector config (OTLP in, 4 exporters out)
│   ├── grafana/provisioning/         # Datasources + dashboards (auto-loaded)
│   └── README.md                     # Stack-specific details
├── docker-compose.yml                # 6-service observability stack
