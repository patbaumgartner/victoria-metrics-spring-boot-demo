...
Start the observability stack from the project root, then run the app on your host:

```bash
docker compose up -d        # from the project root: starts the collector + Victoria backends
cd app
./mvnw spring-boot:run      # the app pushes telemetry to the collector on localhost:4318
```
...
Metric names follow Spring Boot 4.1 OTLP/Micrometer naming (dot notation) in
VictoriaMetrics, for example:

- `http.server.requests_count` - HTTP request count
- `http.server.requests_bucket` / `http.server.requests_sum` - latency histogram
- `jvm.memory.used` - JVM memory usage
- `demo.fetch-data.max` - custom `@Observed` metric

The exact set depends on active endpoints and traffic.
...
### Logs missing in Grafana Logs Drilldown

The Logs Drilldown app requires a Loki datasource (`type=loki`).

```bash
curl -s http://localhost:3100/ready
curl -s -u admin:admin123 'http://localhost:3000/api/datasources' | jq -r '.[] | [.name,.type,.uid] | @tsv' | grep -E '^Loki\tloki\tloki$'
```
