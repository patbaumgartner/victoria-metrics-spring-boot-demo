...
2. Run the Spring Boot app on your host:
...
- Loki (`3100`) for Grafana Logs Drilldown compatibility
- OpenTelemetry Collector (`4317`, `4318`, `13133`) for ingest/routing
- Grafana (`3000`) for unified visualization
- Spring Boot demo app (`8080`) — runs on the host, generating telemetry
...
- Loki API/health: http://localhost:3100
- OTel Collector health: http://localhost:13133

## Open Dashboards In Grafana

Provisioned dashboards are stored in the `dashboards` folder.

1. Open Grafana.
2. Go to Dashboards > Browse.
3. Open folder `dashboards`.

Direct links:

- Spring Boot Application Metrics: http://localhost:3000/d/spring-boot-metrics
- Application Reliability & SLOs: http://localhost:3000/d/reliability-dashboard
...
- VictoriaMetrics (Prometheus API) (`prometheus`)
- VictoriaTraces (Tempo API) (`tempo`)
- Loki (`loki`)
